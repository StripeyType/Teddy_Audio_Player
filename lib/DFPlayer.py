...
class DFPlayer(object):
  ...
  def _write_data(self, cmd, dataL=0, dataH=0, feedback=0):
    # self._uart.write(b'\x7E')        # Begin Message
    # self._uart.write(b'\xFF')        # Firmware version
    # self._uart.write(b'\x06')        # Command length
    # self._uart.write(bytes([cmd]))   # Command word
    # self._uart.write(b'\x01')        # Feedback flag
    # self._uart.write(bytes([dataH])) # DataH
    # self._uart.write(bytes([dataL])) # DataL
    # self._uart.write(b'\xEF')        # End of Message
    _message = bytearray(b'\x7E\xFF\x06\x00\x00\x00\x00\xEF')
    _message[3] = cmd
    _message[4] = feedback
    _message[5] = dataH
    _message[6] = dataL
    self._uart.write(_message)
  ...
  def play(self,folder=None,track=None):
    if folder is None and track is None:
      self._write_data(0x0D)                               # playback (unpause)
    elif folder is None:
      self._write_data(0x03,int(track%256),int(track/256)) # specify track in cwd
    elif track is None:
      self._write_data(0x12,folder)                        # specify folder/album
    else:
      self._write_data(0x0F,folder,track)                  # docs say "specify folder" but may be incorrect
